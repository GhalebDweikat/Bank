...
public class BankFrm extends JFrame
{
	...
	public void setMediator(BankGUIMediator mediator) {
...
//        rowdata = new Object[8];
	...
	void updateModel(DefaultTableModel m)
	{
		JTable1.setModel(m);
	}
...
//            model.addRow(rowdata);
			mediator.personPartyAdded(clientName, street, city, state, zip, "", "");
			if(accountType == "Ch")
				mediator.checkingAccountAdded(clientName, Integer.parseInt(accountnr));
			else
				mediator.savingsAccountAdded(clientName, Integer.parseInt(accountnr));
			updateModel(mediator.getModel());
...
//            rowdata[5] = "0";
//            model.addRow(rowdata);
			mediator.organizationPartyAdded(clientName, street, city, state, zip, "", 0);
			if(accountType == "Ch")
				mediator.checkingAccountAdded(clientName, Integer.parseInt(accountnr));
			else
				mediator.savingsAccountAdded(clientName, Integer.parseInt(accountnr));
	...
	void JButtonDeposit_actionPerformed(ActionEvent event)
	{
        ...
        if (selection >=0){
			...
			String clientnm = (String)model.getValueAt(selection, 1);
            ...
            mediator.doDeposit(clientnm, Integer.parseInt(accnr), deposit);
            updateModel(mediator.getModel());
...
//		    model.setValueAt(String.valueOf(newamount),selection, 5);
	...
	void JButtonWithdraw_actionPerformed(ActionEvent event)
	{
        ...
        if (selection >=0){
		    ...
		    // compute new amount
			double deposit = Double.parseDouble(amountDeposit);
			mediator.doWithdraw(clientnm, Integer.parseInt(accnr), deposit);
            updateModel(mediator.getModel());
